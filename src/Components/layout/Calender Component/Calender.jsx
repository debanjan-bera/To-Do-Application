...
export const CalenderComponent = () => {
  ...
  const activeDateColor = (date) => {
    ...
    return isSunday ? "text-red-500 hover:bg-red-100" : "text-black hover:bg-sky-100";
  };
    ...
    <>
      <aside className="calender bg-[#1E1F24] row-start-2 row-end-5 col-start-3 text-white text-base flex flex-col items-center">
        <section className="w-[94%] bg-white grid text-black p-2 rounded-md">
          ...
          <div className="text-lg font-semibold cursor-pointer select-none grid grid-cols-7 gap-1 text-center">
            {listOfDays.map((day) => (
              <div key={day} className="text-base text-gray-700 px-[0.3rem]">{day}</div>
            ))}
            {[...Array(firstDateOfMonth).keys()].map((index) => (
              <span key={`empty-${index}`} className="text-black/50 p-[0.3rem] rounded-full border border-white scale-100 cursor-pointer select-none transition-all">
            ...
            {[...Array(6 - lastDayOfMonth).keys()].map((index) => (
              <span key={`next-${index}`} className="text-neutral-500 p-[0.3rem] rounded-full border border-white scale-100 cursor-pointer select-none transition-all">
                {index + 1}
              </span>))}
          </div>
        </section>
