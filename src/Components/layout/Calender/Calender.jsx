      ...
      <section className="w-[20rem]">
        ...
        >
          ...
          {Array.from({ length: lastDay }, (_, i) => i + 1).map((day) => (
            <div
              key={day}
              className={`current-month w-full h-full p-1 rounded-full flex items-center justify-center aspect-square ${trackTodayDate(day)} hover:bg-[#27272a]`}
            >
              {day}
            </div>
          ))}
          ...
          {nextMonthDays.map((day, index) => (
            <div key={`next-${index}`} className="cursor-not-allowed text-gray-400 flex items-center justify-center">{day}</div>
          ))}
        </section>
      </section>
...
//                   // onClick={() => setDate(`${day},${month + 1},${year}`)}
//                   >
//                   {day}
//                 </div>))}
