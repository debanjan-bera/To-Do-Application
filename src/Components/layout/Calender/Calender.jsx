...
export const CalenderComponent = () => {
  ...
  const disableDayClass = 'p-1 w-full h-full aspect-square text-gray-400 rounded-full hover:bg-[#27272a]';
  ...
  return (
    <>
      <aside className='calender bg-[#0B0D0E] border-l-[0.02rem] border-neutral-700 justify-items-center row-start-2 row-end-5 col-start-3 w-full grid'>
        <section className='h-full w-[20rem]'>
          <div className="p-2 w-[102%] pt-2 bg-white/10 text-white text-xl"
          onClick={()=>{
          ...
          <section className='w-full h-[20.4rem] text-lg text-white grid grid-cols-7 grid-rows-7 gap-1'>
            ...
            {prevMonthDates.map((day, index) => (
              <div key={`prev-${index}`} className="">
                <div className={`${disableDayClass} ${flexClass}`}>
                {day}
                </div>
              </div>
            ))}
            ...
            {Array.from({ length: lastDay }, (_, i) => i + 1).map((day, index) => (
              <div key={index} className={`w-full h-full p-1 rounded-full ${flexClass} aspect-square ${trackTodayDate(day)}  hover:bg-[#27272a]`}> 
                {day}
              </div>
            ))}

            {nextMonthDays.map((day, index) => (
              <div key={`prev-${index}`} className={`${disableDayClass} ${flexClass}`}>{day}</div>
            ))}
          </section>
        </section>
      </aside>
