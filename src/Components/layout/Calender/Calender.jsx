...
import {IoMdArrowDropleftCircle,IoMdArrowDroprightCircle,
} from "react-icons/io";
...
export const CalenderComponent = () => {
  ...
  const trackTodayDate = (date) => {
    ...
    const isTargetDate =targetDate === date &&month === todayDate.getMonth() &&year === todayDate.getFullYear();
  ...
  useEffect(() => {
    const sundays = [];
    for (let day = 1; day <= lastDay; day++) {if (new Date(year, month, day).getDay() === 0) sundays.push(day);}
    setSunday(sundays);
  }, [year, month, lastDay]);
      ...
      <section className="w-[20rem]">
        ...
        <section className="w-full h-[20.4rem] mb-1 text-lg text-white grid grid-cols-7 grid-rows-7 gap-1 cursor-pointer" onClick={handleDateClick}>
          ...
          {Array.from({ length: lastDay }, (_, i) => i + 1).map(
            (date, index) => (
              <div key={index} className={`current-month rounded-full ${flexClass} aspect-square ${trackTodayDate( date)} hover:bg-[#27272a]`}>
                {date}
              </div>
            )
          )}
