...
export const AddaskForm = () => {
  ...
  const handleFromInput = (event) => {
    ...
    name === "content" ? setInputValue((prevData) => ({...prevData,[name]: value,id: taskId,})): setInputValue((prevData) => ({ ...prevData, [name]: value }));
  };
  const handleFromSubmit = (e) => {
    e.preventDefault();
    if (!content) return;
    const ifTododMatchedId = taskArr.find((currentTask) => currentTask.id === id);
    const ifTododMatchedContent = taskArr.find(
  ...
  return (
    <section
      className={` register-cont w-lvw h-lvh absolute top-0 left-0 bg-red-200/50 z-10  flex items-center justify-center `}
    >
      <form
        action=""
      ...
      >
        ...
        <label htmlFor="">
          ...
          <input
            ...
            name="content"
            value={content}
            onChange={(event) => handleFromInput(event)}
          />
        </label>
        <label htmlFor="">
