...
export const AddTaskForm = () => {
  ...
  return (
    <section className="register-cont w-lvw h-dvh absolute top-0 left-0 bg-black/70 z-10 flex items-center justify-center">
      <motion.form ref={modalRef} className="bg-neutral-100 p-4 rounded-md" initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 25 }} drag="y" dragControls={controls} dragConstraints={{ top: 0, bottom: 0 }} onSubmit={handleSubmit(onSubmit)}>
        ...
        <label>
          ...
          <input type="text" placeholder="Add your important task..." autoFocus
            className="w-[22rem] p-[0.5rem] outline-none text-xl rounded border-[1.5px] border-gray-400"
            autoComplete="off" {...register("content", { required: true })}/>
        ...
        {selectedGroup === "Others" && (
          <label>
            ...
            <input type="text" placeholder="Enter custom group name..."
              className="w-[22rem] p-[0.5rem] outline-none text-xl rounded border-[1.5px] border-gray-400"
